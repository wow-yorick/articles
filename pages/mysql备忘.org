...
#+BEGIN_SRC sql
update ticket_version set ext_attr = JSON_SET(ext_attr, '$.commission_rate', 6) where `user_id` =1205645 and json_extract(ext_attr,"$.commission_rate") = 7.5

-- 清空数据
TRUNCATE TABLE ticket_code_receive;

-- 更新json字段的值
`ext_attr` = JSON_SET(`ext_attr`,
    '$.pay_amount', 2,
    '$.cash_amount', 2
  )
